...
; Try 6: simpler version, without string
...
; The literal punc chars are teh once that participate in multiple operators.
; ASCII punc: !"#$%&'()*+,-./:;<=>?@[\]^_`{|}~
...
.const E_G T_BAREWORD   ; first letter
.const E_S T_BAREWORD   ; subsequent letter
.const E_T T_BAREWORD   ; subsequent digit

.const E_H T_NUM    ; first digit
.const E_U T_NUM    ; subsequent digit

.const E_I EMIT_CHAR    ; P = <operpunc>
.const E_V T_TERN1

.const E_W T_IDENT  ; sigil + 1 char
.const E_Z T_IDENT
.const E_AA T_IDENT

.const E_L T_IGNORE ; whitespace
.const E_X T_IGNORE ; ditto
